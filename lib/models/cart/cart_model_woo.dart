...
import '../entities/index.dart'
    show Product, ProductVariation, Tax, PWGiftExt, YithAddonsOption;
...
class CartModelWoo
  ...
  double getTotal() {
    ...
    subtotal -= loyaltyCoupon?.value ?? 0;
  ...
  double? getPaymentTotal() {
    if (wooSmartCod?.isRiskFreeEnabled == true) {
      return wooSmartCod?.userAdvanceAmount;
    }
    return super.getPaymentTotal();
  }
  ...
  double getProductPrice(id) {
    ...
    if (cartItemMetaDataInCart[id]?.selectedYithOptions != null) {
      var basePrice = cartItemMetaDataInCart[id]!.variation?.valuePrice ??
          item[productId]!.valuePrice;
      return YithAddonsTools.getSelectedYithOptionsPrice(
              basePrice,
              cartItemMetaDataInCart[id]!.selectedYithOptions!,
              productsInCart[id]!) +
          basePrice * productsInCart[id]!;
    }
...
// Removes an item from the cart.
  ...
  Future<void> clearCart({isSaveRemote = true, isSaveLocal = true}) async {
    ...
    if (isSaveRemote) {
      unawaited(Services().widget.syncCartToWebsite(this));
    }
  }
  ...
  FutureOr<(bool, String)> addProductToCart({
    ...
    if (isWalletCart()) {
      await clearCart(isSaveRemote: isSaveRemote);
    }

    var message = '';

    message = _validateProductComponent(product, cartItemMetaData);
    if (message.isNotEmpty) return (false, message);
    ...
    if (product.store != null) {
      final isClosed = (product.store!.storeHour != null) &&
          (product.store!.storeHour!.isDisablePurchase! &&
              !product.store!.storeHour!.isOpen());
      if (isClosed) {
        message = S.of(context).storeClosed;
      ...
      if (product.store?.vacationSettings != null) {
        final isOnVacation = (product.store!.vacationSettings!.vacationMode &&
            ...
            product.store!.vacationSettings!.disableVacationPurchase);
        if (isOnVacation) {
          message = S.of(context).onVacation;
          return (false, message);
        }
      }
    }
    ...
    try {
      ...
      if (cartItemMetaData?.selectedYithOptions?.isNotEmpty ?? false) {
        var options = <YithAddonsOption>[];
        for (var item in cartItemMetaData!.selectedYithOptions!.values) {
          options.addAll(item.values);
        }
        key +=
            '+${options.map((e) => e.inputValue?.toString() ?? e.label ?? '').join('+')}';
      }
      ...
      if (message.isEmpty) {
        ...
        cartItemMetaDataInCart[key] = cartItemMetaData?.copyWith(
            bookingInfo: product.bookingInfo,
            addonsOptions:
      ...
      unawaited(updateDiscount(onFinish: notifyListeners));
      notifyListeners();
