...
// String? rating;
  ...
  ///
  /// If the product has a sale price, return the sale price
  ///
  /// If the product does not have a sale price, return the regular price
  ///
  ...
  Product.fromJson(Map parsedJson) : id = parsedJson['id'].toString() {
    try {
      ...
      if (yithBadgesMeta.isNotEmpty &&
        ...
        badges = List.from(yithBadgesMeta['value'])
            .map<YITHBadge>((e) => YITHBadge.fromJson(e))
            .toList();
      }
      ...
      if (parsedJson['meta_data'] != null) {
        ...
        if (type == 'auction') {
          try {
            auctionInfo =
                ProductAuction.fromProductMetaData(parsedJson['meta_data']);
          } catch (_) {}
        }
      }
...
// In case 1234+https://somelink.com/might-have-dash-character-here
  ...
  Product.fromListingJson(Map<String, dynamic> json)
    ...
    try {
      isListing = true;
      name =
          Tools.getValueByKey(json, DataMapping().kProductDataMapping['title'])
              ?.toString()
              .unescape();
      ...
      if (pureTaxonomies != null) {
        final listingCategories = pureTaxonomies?['listing_category'];
        if (listingCategories is List) {
          for (var categoryData in listingCategories) {
            if (categoryData is Map<String, dynamic> &&
                categoryData['slug'] != null) {
              categories.add(Category.fromListingJson(categoryData));
            }
          }
        }
      }
      ...
      tiktok = Tools.getValueByKey(
          json, DataMapping().kProductDataMapping['tiktok']);
  ...
  Product.fromServerlessJson(Map json) : id = json['id'].toString() {
    ...
    try {
      ...
      categories =
          parsedJson['categories'] != null && parsedJson['categories'] is List
              ? (parsedJson['categories'] as List)
                  ...
                  .toList()
              : [];
      variationIds = parsedJson['ProductVariations'] != null &&
          ...
          : null;
      if (isVariableProduct) {
        onSale = (salePrice?.isNotEmpty ?? false) &&
      ...
      if (isVariableProduct && variationProducts != null) {
        ...
        final listSalePrice =
            variationProducts!.map((e) => e.salePrice).toList();
      ...
      categoryIds = parsedJson['CategoryIds'] != null &&
      ...
      if (parsedJson['PureTaxonomies'] is Map) {
        ...
        // Process categories
        if (taxonomies['listing-category'] is List &&
            (taxonomies['listing-category'] as List).isNotEmpty) {
          categories = [];
          categoryIds = [];
          for (var category in taxonomies['listing-category']) {
            if (category is Map) {
  ...
  Map<String, dynamic> toFirestoreMap() {
    ...
    addIfNotNull(
        ...
        variationProducts
            ...
            .toList());
    addIfNotNull(map, 'ProductAttributes',
        attributes?.map((attribute) => attribute.toFirestoreMap()).toList());
    addIfNotNull(
        map, 'Brand', brands.isNotEmpty ? brands.first.toFirestoreMap() : null);
    ...
    addIfNotNull(map, 'PriceRange', priceRange);
