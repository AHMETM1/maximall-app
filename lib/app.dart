...
import 'screens/my_rating/index.dart';
...
class AppState extends State<App>
  ...
  ThemeData getTheme({
    ...
    setSystemUiOverlay(isDarkTheme);
  ...
  void _loadLocalCart() async {
    await cartModel.model.clearCart(isSaveRemote: false, isSaveLocal: false);
    cartModel.model.getCartInLocal(context);
  }
  ...
  Future<void> onLoaded(User? user) async {
    ...
    // Set logged in user in CartModel
    cartModel.model.setUser(user);

    // If not has cookie, load cart from local
    if (user?.cookie == null) {
      _loadLocalCart();
    } else {
      try {
        final isSuccess = await Services()
            .widget
  ...
  Future<void> onLogout(User? user) async {
    unawaited(cartModel.model.clearCart(isSaveRemote: false));
  ...
  void onMessage(FStoreNotificationItem notification) {
    ...
    //reload my points for points offline store / loyalty
    if (notification.additionalData?['type'] == 'points_added' ||
        notification.additionalData?['type'] == 'points_redeemed') {
      eventBus.fire(EventReloadMyPoints(notification));
    }
  }
          ...
          return Directionality(
            ...
            child: MultiProvider(
              providers: [
                ...
                ChangeNotifierProxyProvider<UserModel,
                    ListPurchasedProductModel>(
                  create: (context) {
                    return ListPurchasedProductModel(user: _user.user);
                  },
                  update: (_, userModel, ___) {
                    return ListPurchasedProductModel(user: userModel.user);
                  },
                )
              ],
              child: MaterialApp(
  ...
  void onMessageOpenedApp(FStoreNotificationItem notification) async {
    ...
    if (url != null) {
      await Services().dynamicLinkService.handleLink(url);
    }
    _notificationModel.saveMessage(notification);
  }
}
